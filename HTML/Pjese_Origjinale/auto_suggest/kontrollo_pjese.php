...
if (kontrolloMercedes($part)) {
    echo json_encode(["marka" => "Mercedes-Benz", "mesazh" => "✅ Pjesa ka format origjinal Mercedes-Benz ($part)."]);
} elseif (kontrolloBMW($part)) {
    echo json_encode(["marka" => "BMW", "mesazh" => "✅ Pjesa ka format origjinal BMW ($part)."]);
} elseif (kontrolloAudi($part)) {
    echo json_encode(["marka" => "Audi", "mesazh" => "✅ Pjesa ka format të mundshëm origjinal Audi ($part)."]);
} elseif (kontrolloVW($part)) {
    echo json_encode(["marka" => "Volkswagen", "mesazh" => "✅ Pjesa ka format të mundshëm origjinal Volkswagen ($part)."]);
} else {
    echo json_encode(["marka" => "Unknown", "mesazh" => "❌ Nuk përputhet me formatet e zakonshme të Mercedes, BMW, Audi ose VW."]);
}
?>
